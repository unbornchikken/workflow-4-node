...
WorkflowInstance.prototype._addIdleInstanceIdPathTracker = function () {
    ...
    let tracker = {
        activityStateFilter: function (args) {
            return args.reason === enums.activityStates.cancel ||
                (
                    self._idleInstanceIdPathCallback &&
                    args.scope.$activity instanceof BeginMethod &&
                    _.isString(args.scope.methodName) &&
        ...
        activityStateChanged: function (args) {
            if (args.reason === enums.activityStates.idle) {
                let methodName = args.scope.methodName.trim();
                ...
                if (specStrings.hosting.isDelayToMethodName(methodName)) {
                    // TODO - fix this, replace the existing instance. Investigate why existing instances aren't cleaned up properly.
                    const existing = _.find(self.activeDelays, (ad) => ad.methodName === methodName);
                    if (true || !existing) {
            ...
            } else if (args.reason === enums.activityStates.cancel) {
                let methodName = args.scope.methodName && args.scope.methodName.trim();
                if (specStrings.hosting.isDelayToMethodName(methodName)) {
                    console.log("buzinga");
                }
            }
