...
class BaseIterator extends Activity {
  ...
  _bodyFinished(callContext, result, reason) {
    if (result === Activity.states.complete) {
      callContext.activity._doStep.call(this, callContext, result, reason);
      return;
    } else if (result === Activity.states.fail) {
      if (reason instanceof IgnoredByTryCatchEvent) {
        if (reason.event instanceof LoopBreakEvent) {
          callContext.complete();
          return;
        }
        if (reason.event instanceof LoopContinueEvent) {
          callContext.activity._doStep.call(this, callContext, result, reason);
          return;
        }
      }
